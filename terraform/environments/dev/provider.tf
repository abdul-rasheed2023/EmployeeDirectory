...
# Same auth pattern as the kubernetes provider above — needed to install the
# AWS Load Balancer Controller Helm chart, which the ingress resource below
# depends on to actually provision an ALB.
provider "helm" {
  kubernetes {
    host                   = module.eks.cluster_endpoint
    cluster_ca_certificate = base64decode(module.eks.cluster_certificate_authority_data)
